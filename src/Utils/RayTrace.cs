...
public static class RayTrace
{
    ...
    public static unsafe Vector? TraceShape(Vector? start, Vector end, nint skip = 0, ulong mask = 12289UL, bool show = false)
    {
        ...
        DrawResult(start, end, result, show);
    ...
    private static void DrawResult(Vector start, Vector end, bool result = false, bool show = false)
    {
        if (show)
        {
            ...
            Utils.DrawBeam(start, end, color, 5);
        }
    }
}
